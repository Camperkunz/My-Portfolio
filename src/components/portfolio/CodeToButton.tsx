...
const PAUSE_BEFORE_TRANSFORM = 600;
...
export default function CodeToButton({ onNavigate }: Props) {
  ...
  return (
    <div className="relative inline-flex items-center justify-center min-w-[200px] h-10">
      <AnimatePresence mode="wait">
        {phase !== "button" ? (
          ...
          <motion.button
            key="button"
            onClick={onNavigate}
            className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium h-10 px-4 py-2 border border-accent/40 text-foreground bg-background hover:bg-accent hover:text-accent-foreground hover:border-accent transition-all"
